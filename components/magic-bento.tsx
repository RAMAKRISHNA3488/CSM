...
export interface BentoCardProps {
  ...
  label?: string
  image?: string
  // For cards with multiple images (like card 8)
...
const defaultCardData: BentoCardProps[] = [
  ...
  // Card 9: Web Designing Info (2 cols, 1 row)
  {
    ...
    description:
      "Our web design team brings creativity and technical expertise to build stunning, responsive websites that drive results.",
    label: "WEB DESIGNING",
  },
]
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const renderCard = (card: BentoCardProps, index: number, gridClass: string) => {
    ...
    if (card.images && card.images.length > 0) {
      if (enableStars) {
        return (
          <ParticleCard
            ...
            style={{ ...cardStyle, backgroundImage: undefined }}
          ...
          >
            <div className="card__header flex justify-between gap-3 relative text-white z-10">
              ...
              <span className="card__number text-xs text-white/40">#{index + 1}</span>
            </div>
            <div className="flex gap-3 flex-1 items-center justify-center py-4">
              {card.images.map((img, i) => (
                <div key={i} className="flex flex-col items-center gap-2">
                  ...
                  />
                  <span className="text-sm font-semibold text-white">{img.name}</span>
                </div>
              ))}
            </div>
          </ParticleCard>
      ...
      return (
        <div key={index} className={baseClassName} style={{ ...cardStyle, backgroundImage: undefined }}>
          <div className="card__header flex justify-between gap-3 relative text-white z-10">
            <span className="card__label text-xs uppercase tracking-wider text-white/60">{card.label}</span>
            <span className="card__number text-xs text-white/40">#{index + 1}</span>
          </div>
      ...
      return (
        ...
        >
          ...
          <div className="card__content flex flex-col relative text-white z-10 mt-auto">
            <h3 className="card__title font-semibold text-lg md:text-xl m-0 mb-2">{card.title}</h3>
            <p className="card__description text-sm leading-relaxed text-white/70">{card.description}</p>
          </div>
        </ParticleCard>
      )
    }
    ...
    return (
      <div key={index} className={baseClassName} style={cardStyle}>
        {card.image && <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />}
        <div className="card__header flex justify-between gap-3 relative text-white z-10">
          <span className="card__label text-xs uppercase tracking-wider text-white/60">{card.label}</span>
          <span className="card__number text-xs text-white/40">#{index + 1}</span>
        </div>
  ...
  return (
    <>
      ...
      <div className="bento-section p-3 w-full max-w-6xl mx-auto select-none relative" ref={gridRef}>
        <div className="bento-grid">
          {cards.slice(0, 9).map((card, index) => renderCard(card, index, getGridClass(index)))}
        </div>
      </div>
    </>
  )
